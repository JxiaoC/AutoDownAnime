...
if (!window.localStorage.JJ_Token) {
    ...
    if (window.location.href.indexOf('mobile') == -1 && !window.localStorage.RightAndBotton_Time || window.localStorage.RightAndBotton_Time != nowtime) {
        //↘
        document.write('<div style="position: fixed;right: 0;bottom: -1px;z-index:5;"><div style="cursor: pointer;" onclick="$(this).parent().remove();">关闭<\/div><a href="http://t.cn/RQ2GA29" target="_black"><img src="http://zcdn.jijidown.com/static/img/right_bottom.gif"><\/a><\/div>');
        setTimeout(function () {
...
function yimada(){
        if ($('#go').val()) {
            $('.down_msg').html('姨妈大! 按下<span style="font-weight: 700;color:#ffb000;">回车</span>!');
        } else {
            $('.down_msg').html('输入B站视频下载地址或任意字符搜索');
        }
}